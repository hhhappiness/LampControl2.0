...
: SubPage(ObjNum)
{
	if(IsLanguageCh()) {
		...
		ObjList[iWorkTime]->SetPos(2*DEFAULT_HANZI_WIDTH,0);
		ObjList[iPowerOffDelay]->SetPos(2*DEFAULT_HANZI_WIDTH,16);
		...
		ObjList[iWorkTime]->SetPos(11*DEFAULT_ASCII_WIDTH,0);
		ObjList[iPowerOffDelay]->SetPos(11*DEFAULT_ASCII_WIDTH,16);
	}
...
void PowerSetupPage::Show()
{
	...
	if(IsLanguageCh()) {
		...
		DispStr8(0,0,"持续   分钟后暂停");
		DispStr8(0,16,"暂停   分钟后关机");
	}
	else {
		//DispStr8(8*DEFAULT_HANZI_WIDTH+7,0,",");
		// DispStr8(0,16,"Stop After:    Min");
		// DispStr8(0,32,"Off  After:    Min");
		DispStr8(0,0,"Stop After:    Min");
		DispStr8(0,16,"Off  After:    Min");
	}
	//显示控件
	GUI_Page::Show();
	//刷新到Lcd
