...
#define E_KEY (&aes_key.buf[0])
#define D_KEY (&aes_key.buf[60])
...
void aes_encrypt(u8 *out, const u8 *in)
{
	...
	if (aes_key.key_length > 24) {
		f_nround (b1, b0, kp);
		f_nround (b0, b1, kp);
	}

	if (aes_key.key_length > 16) {
		f_nround (b1, b0, kp);
		f_nround (b0, b1, kp);
	}
...
void aes_decrypt(u8 *out, const u8 *in)
{
	...
	__le32 *dst = (__le32 *)out;
	u32 b0[4], b1[4];
	const int key_len = aes_key.key_length;
